...
float NodeHybrid::getTraversalCost(const NodePtr &child)
{
  ...
  if (si && si->use_direction_map && dir_map && dir_map->isValid() && si->direction_heading_weight > 0.0)
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("smac_planner"),
                "[DirectionMap] Entered direction-aware cost computation at (%.2f, %.2f)", child->pose.x, child->pose.y);

    // Convert child pose (cell index ) to world coordinates (meters)
    double child_wx, child_wy;
    dir_map->mapToWorld(static_cast<unsigned int>(child->pose.x),
                        static_cast<unsigned int>(child->pose.y),
                        child_wx, child_wy);

    double parent_wx, parent_wy;
    dir_map->mapToWorld(static_cast<unsigned int>(this->pose.x),
                        static_cast<unsigned int>(this->pose.y),
                        parent_wx, parent_wy);

    unsigned int mx, my;
    if (!dir_map->worldToMap(child_wx, child_wy, mx, my)) 
    {
      // Extended debug info to check if the child pose is outside direction map bounds
      const double ox = dir_map->getOriginX(); //-31.90
      const double oy = dir_map->getOriginY(); //-18.80
      const double res = dir_map->getResolution(); //0.05
      const unsigned int width = dir_map->getSizeInCellsX(); //1033
      const unsigned int height = dir_map->getSizeInCellsY(); // 1011

      const double x_map_max = ox + width * res; //meter
      const double y_map_max = oy + height * res;
      // const double x_map_max = width -1;
      // const double y_map_max = height -1;

      RCLCPP_WARN(
          rclcpp::get_logger("smac_planner"),
          "[DirectionMap] worldToMap failed for child (wx=%.2f, wy=%.2f), "
          "DirectionMap origin=(%.2f, %.2f), size: %u x %u, res: %.3f, bounds x=[%.2f, %.2f], y=[%.2f, %.2f]",
          child_wx, child_wy,
          ox, oy, width, height, res,
          ox, x_map_max, oy, y_map_max);

      if (child_wx < ox || child_wx > x_map_max ||
          child_wy < oy || child_wy > y_map_max)
      {
        RCLCPP_WARN(rclcpp::get_logger("smac_planner"),
                    "[DirectionMap] Child pose is outside the direction_map bounds!");
      }
      else
      {
        RCLCPP_WARN(rclcpp::get_logger("smac_planner"),
                    "[DirectionMap] Child pose is inside bounds, but worldToMap still failed!");
      }

      return travel_cost; // skip directional penalty if mapping fails
    }
    // --- Robust bounds check using new isInsideMap utility ---
    if (dir_map->isInsideMap(mx, my))
    {
      RCLCPP_INFO(
                rclcpp::get_logger("smac_planner"),
                "Inside map.....................................");
      ...
      if (dir_map->getPreferredTheta(mx, my, theta_ref))
      {
        ...
        const float dx = static_cast<float>(child_wx - parent_wx);
        const float dy = static_cast<float>(child_wy - parent_wy);
        if (std::hypot(dx, dy) > 1e-6f)
        {
          RCLCPP_INFO(
                rclcpp::get_logger("smac_planner"),
                "check.....................................");
          ...
          // Normalize Δθ to [0,1] by π, then apply quadratic penalty.
          const float norm = std::fabs(dtheta) / static_cast<float>(M_PI); // [0,1]
          const float W = static_cast<float>(si->direction_heading_weight);
          const float heading_factor = 1.0f + W * std::pow(norm, 2);
          if (std::fabs(dtheta) > 2.0f)
          {
            RCLCPP_INFO(
                rclcpp::get_logger("smac_planner"),
                "[DIR PENALTY] Opposing motion at (%u,%u): θ_ref=%.1f°, θ_motion=%.1f°, Δθ=%.2f rad, factor=%.2f",
                mx, my,
                theta_ref * 180.0f / static_cast<float>(M_PI),
                theta_motion * 180.0f / static_cast<float>(M_PI),
                dtheta, heading_factor);
          }
          ...
          RCLCPP_INFO(
              rclcpp::get_logger("smac_planner"),
              "[HeadingPenalty] Cell (%u,%u) → θ_ref=%.1f°, θ_motion=%.1f°, Δθ=%.2f rad, factor=%.2f",
              mx, my,
              theta_ref * 180.0f / static_cast<float>(M_PI),
              theta_motion * 180.0f / static_cast<float>(M_PI),
              dtheta, heading_factor);
        ...
        {
          RCLCPP_INFO(rclcpp::get_logger("smac_planner"),
                      "[HeadingPenalty] skipped: motion Δx/Δy too small at (%u,%u)", mx, my);
        }
      }
      else
      {
        RCLCPP_INFO(rclcpp::get_logger("smac_planner"),
                    "[HeadingPenalty] skipped: preferred theta not available at (%u,%u)", mx, my);
      }
    }
    else
    {
      RCLCPP_WARN(rclcpp::get_logger("smac_planner"),
                  "[DirectionMap] Map index out of bounds: mx=%u, my=%u (size: %u x %u)", mx, my,
                  dir_map->getSizeInCellsX(), dir_map->getSizeInCellsY());
    }
  }
  // Final cost print (optional)
  RCLCPP_INFO(rclcpp::get_logger("smac_planner"),
              "Total traversal cost for child (%.2f, %.2f): %.3f",
...
struct SearchInfo
{
  ...
  // --- Heading gate ---
  // float max_heading_deviation_rad = static_cast<float>(M_PI * 0.5f); // default 90°
...
namespace nav2_smac_planner
{
  ...
  public:
    ...
    // --- 3. Utility: check if map indices are inside map bounds ---
    ...
    bool getPreferredTheta(unsigned int mx, unsigned int my, float &theta) const
    {
      ...
      // const int8_t s = data_[idx(mx, my)];
      // RCLCPP_INFO(rclcpp::get_logger("direction_map"),
      //             "getPreferredTheta: mx=%d, my=%d → raw=%d", mx, my, s);

      size_t index = idx(mx, my);
      RCLCPP_INFO(rclcpp::get_logger("direction_map"),
                  "Index calculation: %zu = %u * %u + %u", index, my, size_x_, mx);

      const int8_t s = data_[index];
      RCLCPP_INFO(rclcpp::get_logger("direction_map"),
                  "Raw value at index %zu: %d", index, s);
      ...
      // Treat 0 as "no preference"; only 1..99 are headings.
      const int v = (u + 256) % 256;
        RCLCPP_INFO(rclcpp::get_logger("direction_map"),
                  "getPreferredTheta: mx=%u, my=%u → raw=%d (wrapped=%d)", mx, my, u, v);

      if (v == 0 || v > 99)
        return false;

      const float norm = static_cast<float>(v) / 100.0f; // 0.01..0.99
      theta = norm * 2.0f * static_cast<float>(M_PI);
    ...
    //   size_t index = idx(mx, my);
    //   const int8_t s = data_[index];
    //   const int v = static_cast<int>(s); // range: -1 to 100
    //   const int u = (v + 256) % 256;     // wrap into 0–255 if needed

    //   RCLCPP_INFO(rclcpp::get_logger("direction_map"),
    //               "getPreferredTheta: mx=%u, my=%u → raw=%d (wrapped=%d)", mx, my, v, u);

    //   if (u == 0)
    //   {
    //     theta = 0.0f; // default forward
    //     return true;
    //   }
    //   else if (u == 25)
    //   {
    //     theta = static_cast<float>(M_PI) / 2.0f; // left
    //     return true;
    //   }
    //   else if (u == 50)
    //   {
    //     theta = static_cast<float>(M_PI); // back
    //     return true;
    //   }
    //   else if (u == 75)
    //   {
    //     theta = 3.0f * static_cast<float>(M_PI) / 2.0f; // right
    //     return true;
    //   }
    //   else if (u >= 1 && u <= 99)
    //   {
    //     theta = (static_cast<float>(u) / 100.0f) * 2.0f * static_cast<float>(M_PI); // normal case
    //     return true;
    //   }

    //   return false; // skip -1 (unknown), 100 (occupied), etc.
    // }
    ...
    // Direct access (if needed for debug)
  ...
  private:
    ...
    // idx helper  // Row-major index helper (y * width + x)
    inline size_t idx(unsigned int x, unsigned int y) const
    {
      return static_cast<size_t>(y) * static_cast<size_t>(size_x_) + static_cast<size_t>(x);
    }
  };
